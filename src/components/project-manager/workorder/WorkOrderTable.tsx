...
const WorkOrderTable = ({ clientId }: { clientId: string }) => {
  ...
  const getAllWorkOrders = async () => {
    ...
    const httpClient = OPHttpClient.init(process.env.V2_API_URL, {
      action: 'Project Manager Work Order Management',
    });
    const apiService = new PMWorkOrderApiService(httpClient);
    apiService.setClientId(clientId);
    const workorders = await apiService.fetchAllWorkOrders();
    if (workorders) {
  ...
  const headCells: HeadCell[] = [
    ...
    //   numeric: false,
    //   disablePadding: false,
    //   label: 'Download Workorder',
    // },
  ];

  const createTableData = () => {
    const showableWorkOrders = workOrders.filter((workorder) => {
      if (workorder.status === 'APPROVED') return true;
      return false;
    });
                      ...
                      <TableRow
                        ...
                        <TableCell align='center'>
                          <Typography
                            ...
                            onClick={async () => {
                              await router.push(
                                `/app/project-manager/work-order/${item._id}`,
                              );
                            }}>
                            <span
                              style={{
                        ...
                        {/* <TableCell align='center'>
                          <Typography
                            variant='button'
                            onClick={() => printWorkorderPdf(()=>console.log())}
                            style={{
