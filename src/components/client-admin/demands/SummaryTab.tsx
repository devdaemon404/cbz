...
const SummaryTab = ({
  ...
  const skills: SkillInformationType[] = [
    ...
    // { name: 'JD File Name', value: jd_file_name || '-' },
  ];
  ...
  const additionalInfo: AdditionalInformationType[] = [
    ...
    // },
    {
      name: 'Employee Type',
      value:
        demandData.summary && demandData.additional_info?.employment_type
          ? demandData.additional_info.employment_type
          : '-',
    },
    // {
    ...
    // },
    {
      ...
      value:
        demandData.summary &&
        // @ts-ignore
        demandData.additional_info?.email_enabled
          ? // @ts-ignore
            'Yes'
          : 'No',
    },
    // { name: 'Status', value: demandData.status || '-' },

    // {
    //   name: 'Location',
    //   value:
    //     // @ts-ignore
    //     demandData.summary && demandData.additional_info.location
    //       ? // @ts-ignore
    //         demandData.additional_info.location
    //       : '-',
    // },
    {
      ...
      value:
        demandData.summary &&
        // @ts-ignore
        demandData.additional_info?.background_check_required
          ? // @ts-ignore
            'Yes'
          : 'No',
    },
          ...
          <Grid container spacing={2} direction='row'>
            <Grid
              ...
              {additionalInfo?.map((info) => {
                ...
                // if (type === 'text-field') {
                return (
                  <Grid container item spacing={2} lg={4} xs={12}>
                    <Grid item xs={6}>
                      <Paper
                        variant='elevation'
                        elevation={0}
