...
export class VADemandApiService {
  constructor(
  ...
  fetchAllDemands = async (): Promise<VendorDemandDataType[] | undefined> => {
    const res = await this.httpClient.get<FetchVendorDemandResponseType>(
      `/apis/v1/demand/vendor/${this.vendorId}`,
    );
    if (isException(res)) {
  ...
  checkRecruitment = async (demandId) => {
    await this.httpClient.post(`apis/v1/demand/${demandId}/recruitment`, {
      recruiter_manager_user_id: this.userId,
      vendor_id: this.vendorId,
    });
    OPToast.show(`Recruitment started for this profile. Click again to start`, {
