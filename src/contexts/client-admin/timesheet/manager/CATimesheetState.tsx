...
import CATimesheetContext from './ca-timesheet-context';
import {
  TimeSheetDataType,
  EmployeeType,
  TaskType,
...
import { CATimesheetApiService } from 'src/apis/client-admin/ca-timesheet-api-service';

const CATimesheetState = (props) => {
  // API URLs
  ...
  // For calling V1 API
  const _apiServiceV1 = new CATimesheetApiService(httpClientV1);
  // For calling V2 API
  const _apiServiceV2 = new CATimesheetApiService(httpClientV2);
  // For employee API
  ...
  // Get all employees
  const getEmployeeList = async (userId, projectId) => {
    setIsLoading(true);
    _apiServiceV1.setUserId(userId);
    const res = await _apiServiceV1.fetchAllEmployees('id');
  ...
  const getEmployeeOfSelectedProject = async (userId) => {
    ...
    // });
    const newEmployeeList = await _apiServiceV1.fetchAllEmployees(
      selectedProject.id,
    );
    if (newEmployeeList) {
...
export default CATimesheetState;
